...
public class MouseLookTest : MonoBehaviour
{
    ...
    private int MouseWheelSensitivity = 10; //滚轮灵敏度设置
    private int MouseZoomMin = 1; //相机距离最小值
    private int MouseZoomMax = 500; //相机距离最大值
    ...
    private float Distance = 130; //相机和target之间的距离，因为相机的Z轴总是指向target，也就是相机z轴方向上的距离
    ...
    void Update()
    {
        ...
        else if (Input.GetAxis("Mouse ScrollWheel") != 0) //鼠标滚轮缩放功能
        {
            ...
            transform.position = storeRotation * new Vector3(-Distance, 25.0F, 0) + CameraTargetPosition;
        }
