...
public class GpuAniMesh_3RHalfExport : IAniMeshExport {
	...
	{
		string filePath = outputFilePath + "/GpuAniMesh3RHalf/";
	...
	public void SaveMeshTexture(string outputFilePath, string AniMeshName)
	{
		...
		int npower = IGGUtil.GettoPower(totalvectors * 3);
		Texture2D combinedTex = new Texture2D(npower, totalFrame, TextureFormat.RHalf, false);
		...
		for (int frame = 0; frame < totalFrame; frame++) 
		{
			for(int k = 0; k < SubMeshVertexNum; k ++)
			{
				...
				combinedTex.SetPixel(k,  frame, new Color(vertex.x, 0, 0));
				combinedTex.SetPixel(k + SubMeshVertexNum, frame,  new Color(vertex.y, 0, 0));
				combinedTex.SetPixel(k + SubMeshVertexNum * 2, frame, new Color(vertex.z, 0, 0));
			}
		}
	...
	public void SaveMesh(string outputFilePath, string MeshName)
	{
		...
		for (int i = 0; i < posArray.Length; i++) {
			posArray[i].x = i * 1.0f / npower;
			posArray[i].y = (i +  totalvectors) * 1.0f / npower;
			posArray[i].z = (i +  totalvectors * 2) * 1.0f / npower;
		}
	...
	{
		Shader s = Shader.Find("GpuCustomUnlit3R");
