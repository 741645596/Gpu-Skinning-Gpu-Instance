...
public class GpuSkinInstance_Export : IAniMeshExport
{
    ...
    {
        string filePath = outputFilePath + "/GpuSkinInstance/";
        ...
        EditorUtility.DisplayDialog("Tip", "GpuSkinInstance Export Complete" + m_AniMesh.m_ActorName, "OK");
    ...
    {
        Shader s = Shader.Find("GpuCustomAlphaSkin");
        ...
        mat.SetTexture("_BoneMatrixTex", AssetDatabase.LoadAssetAtPath(outputFilePath + MaterialName + "_AT.asset", typeof(Texture2D)) as Texture2D);
    ...
    public void SavePrefab(string outputFilePath, Quaternion quaternion, string PrefabName)
    {
        ...
        g.name = PrefabName;
        //
        TestInstance t = g.AddComponent<TestInstance>();
        ...
        t.targetMaterial = AssetDatabase.LoadAssetAtPath(outputFilePath + PrefabName + ".mat", typeof(Material)) as Material;
        t.targetMaterial.enableInstancing = true;
        t.targetMesh = AssetDatabase.LoadAssetAtPath(outputFilePath + PrefabName + "_M.asset", typeof(Mesh)) as Mesh;
        // 保存预知
