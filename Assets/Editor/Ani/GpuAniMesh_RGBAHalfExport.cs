...
public class GpuAniMesh_RGBAHalfExport : IAniMeshExport {
	...
	{
		string filePath = outputFilePath + "/GpuAniMeshRGBAHalf/";
	...
	public void SaveMesh(string outputFilePath, string MeshName)
	{
		...
		Vector3[] Varray = new Vector3[totalvectors];
		for (int i = 0; i < Varray.Length; i++) {
			Varray[i].x = uv[i].x;
			Varray[i].y = uv[i].y;
			Varray[i].z = i * 1.0f / npower;
		}
	...
	{
		Shader s = Shader.Find("GpuCustomUnlitRGBA");
